        ...
        <div class="col-12">
          <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
              <h3 class="text-light">Manage Contact</h3>
  ...
  $(function () {
    ...
    $(document).on('click', '.deleteIcon', function (e) {
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          $.ajax({
            url: "{{ route('contactus.admin.delete') }}",
            method: 'delete',
            data: {
              id: id,
              _token: csrf
    ...
    $(document).on('click', '.viewIcon', function (e) {
      ...
      $.ajax({
        ...
        $("#view_category").html(galleryData.cont_web);
        $("#view_sub_category").html(galleryData.cont_name);
        $("#view_name").html(galleryData.cont_name);
        $("#view_message").html(galleryData.cont_message);
        $("#view_created").html(formattedDate);

        // Use .attr("src", ...) to set the src attribute
        $(".img-holder img").attr("src", `{{asset('storage/images')}}/${galleryData.testst_image}`);

        $("#gallery_id").val(galleryData.id);
        $("#gallery_avatar").val(galleryData.avatar);
        },
    ...
    function fetchAllGalleries() {
      $.ajax({
        url: "{{ route('contact.admin.fetchall') }}",
        method: 'get',
        success: function (response) {
